...
class OpenRgbClientImpl : OpenRgbClient {
    ...
    override fun updateLed(deviceIndex: Int, ledIndex: Int, color: Color) {
        updateLed(
            deviceIndex, ledIndex,
            be.breina.openrgb.model.Color(color.red.toByte(), color.green.toByte(), color.blue.toByte())
        )
    }
    ...
    override fun updateLeds(deviceIndex: Int, vararg colors: Color) {
        updateLeds(
            deviceIndex,
            *Arrays.stream(colors).map { color ->
                be.breina.openrgb.model.Color(color.red.toByte(), color.green.toByte(), color.blue.toByte())
            }
    ...
    private fun updateLed(deviceIndex: Int, ledIndex: Int, color: be.breina.openrgb.model.Color) {
        ...
        sendMessage(
            { cb: CommandBuilder -> cb.command(CommandId.UPDATE_SINGLE_LED).device(deviceIndex) },
            *request.array()
        )
    }
